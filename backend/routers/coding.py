...
from services.ai_coach import chat_with_coach, CODING_SYSTEM_PROMPT, MOCK_INTERVIEW_SYSTEM_PROMPT
...
from services.submission_store import save_submission, get_solved_questions, get_latest_submission, load_submissions, get_question_submissions
...
HINTS_DIR = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "hints")

def _generate_and_cache_hints(question_id: str, title: str, description: str) -> dict:
    """Call Claude to generate 3 progressive hints. Cache the result."""
    os.makedirs(HINTS_DIR, exist_ok=True)
    cache_path = os.path.join(HINTS_DIR, f"{question_id}.json")
    ...
    prompt = (
        f"Problem: {title}\n\n{description}\n\n"
        "Write 3 progressive hints for this coding problem as JSON.\n"
        "hint_1: Algorithm direction only. 1 sentence. No specifics.\n"
        "hint_2: The key insight. 1-2 sentences. More specific, no code.\n"
        "hint_3: Near-solution. 2-3 sentences. Enough to implement, still no code.\n"
        'Respond with ONLY valid JSON: {"hint_1": "...", "hint_2": "...", "hint_3": "..."}'
    )
...
def get_hints(question_id: str):
    """Return 3 progressive hints for a question (generated by Claude, cached on disk)."""
    question = None
    for gq in get_generated_questions():
        if gq["id"] == question_id:
            question = gq
            break
    if not question:
        try:
    ...
    try:
        hints = _generate_and_cache_hints(question_id, question["title"], question["description"])
        return hints
...
def _parse_test_results(stdout: str) -> list[dict]:
    """Parse test runner output lines into structured dicts."""
    import re
    results = []
    for line in (stdout or "").strip().split("\n"):
        m = re.match(r'Test \d+: (PASS|FAIL) \| Input: (.*?) \| Expected: (.*?) \| Got: (.*)', line)
        if m:
            results.append({
...
def chat(req: ChatRequest):
    try:
        prompt = MOCK_INTERVIEW_SYSTEM_PROMPT if req.mock_mode else CODING_SYSTEM_PROMPT
...
@router.get("/submissions/{question_id}/history")
def get_submission_history(question_id: str, user_id: str = "default"):
    """Return all past submissions for a question, newest first."""
    return get_question_submissions(user_id, question_id)
