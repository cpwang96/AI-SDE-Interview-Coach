...
- **Tiered hints** 💡 — 3 progressive hints per problem (generated by Claude, cached on disk); reveal one at a time; hidden in mock mode
- **Test case breakdown** — submit result shows a per-row table (✓/✗, input, expected, got) instead of raw stdout
- **Submission history** 📋 — collapsible panel below notes shows every past attempt (date, language, pass rate)
- **Mock interview mode** 🎯 — 45-min locked timer, hints hidden, coach switches to strict interviewer persona
...
| GET  | `/api/coding/hints/{id}` | Get 3 tiered hints (Claude-generated, cached) |
| GET  | `/api/coding/submissions/{id}/history` | Get all past submissions for a question |
